...
def main():
    ...
    dt_start = dt_end - dt.timedelta(days=252)  # One years
    ...
    #Sharp Ratio
    daily_rets [i] = (close[i]/value[i-1])-1
    std_metric =  np.std(daily_rets)

    import math
    sharp_ratio = math.sqrt(252)*np.mean(daily_rets)
...
def legit_allocations(width=3):
    import itertools
    combs = itertools.combinations(range(11), width)
    for allocations in combs:
        if sum(allocations) == 10:
            alloc = map(lambda x: float(x)/10, allocations)
            yield alloc

for allocation in legit_allocations(4):
    print allocation
